...
# 1. Prevent LangChain internal fallbacks from raising missing OpenAI credential errors
if not os.getenv("OPENAI_API_KEY"):
    os.environ["OPENAI_API_KEY"] = os.getenv("NVIDIA_API_KEY", "nvapi-dummy-key")


# 2. NVIDIA NIM strict payload compatibility: remove content if tool_calls are present
class NvidiaCompatibleChatOpenAI(ChatOpenAI):
...
# 3. Model instance
model = NvidiaCompatibleChatOpenAI(
...
# 4. Middleware that sanitizes tool names from leaked reasoning tokens and catches errors
class TolerateToolErrors(AgentMiddleware):
    """Hand tool failures back to the model as a message so it can recover,
    and sanitize tool names to strip leaked channel/reasoning tokens."""

    async def awrap_tool_call(self, request, handler):
        try:
            raw_name = request.tool_call.get("name", "")
            request.tool_call["name"] = raw_name.split("<|")[0].strip()
...
class Sidekick:
    ...
    async def setup(self):
        ...
        # Bind directly to the existing model instance
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": (
                            f"Your last response did not meet the success criteria. "
